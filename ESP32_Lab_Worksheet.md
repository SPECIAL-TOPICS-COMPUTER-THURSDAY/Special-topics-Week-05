...
__ans__ SRAM เก็บข้อมูลที่ใช้ประมวลผลแบบชั่วคราว (เช่น stack, heap), Flash Memory เก็บโปรแกรมและข้อมูลถาวร.
2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
__ans__ ตัวแปรแบบ global/static อยู่ใน Flash หรือ SRAM ส่วนตัวแปรในฟังก์ชัน อยู่ใน SRAM
3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
__ans__ ESP32 มี SRAM ประมาณ 520KB และ Flash ขึ้นกับโมดูล (เช่น 4MB), ใช้งานจริงขึ้นกับโปรแกรมและการจัดการหน่วยความจำ
---
...
|-----------|-------------|-----------|-------------------|
| Sequential | Internal SRAM | 5635 μs | 1.00x |
| Random | Internal SRAM | 6962 μs | 1.24x |
| Sequential | External Memory | 4971 μs | 0.89x |
| Random | External Memory | 4439 μs | 0.88x |
...
__ans__ Sequential access เร็วกว่าเพราะดึงข้อมูลมาเก็บใน cache ล่วงหน้าได้ต่อเนื่อง (cache-friendly)
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
__ans__ Internal SRAM อยู่ในชิปเร็วกว่า ส่วน external memory อยู่นอกชิปช้ากว่า
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
__ans__ Stride ใหญ่เกินไปทำให้ cache ทำงานไม่ค่อยดี ทำให้ช้าลง
...
| Average Time per Iteration (μs) | 80 | 97506 |
| Total Execution Time (ms) | 5003 | 5942 |
...
**ข้อมูลนักศึกษา:**
- ชื่อ: นางสาว ณัฏฐณิชชา กิติชยาโชติ
- รหัสนักศึกษา: 66030232
...
1. เปรียบเทียบประสบการณ์การใช้ Docker ในสัปดาห์นี้กับสัปดาห์ที่ 4:
   __ans__ เน้นการน้นการวิเคราะห์ architecture มากกว่า มีขั้นตอนที่ซับซ้อนกว่า

2. สิ่งที่เรียนรู้เพิ่มเติมเกี่ยวกับ ESP32 architecture:
   __ans__ ได้จำลอง QEMU ดูการทำงานของ Memory Architecture และCache Performance

3. ความท้าทายที่พบในการทำ architecture analysis:
   โค้ดในตอนที่ 2 ผิดพลาด ทำให้ไม่สามารถ build ได้ในตอนแรก ได้มีการแก้ไขเล็กน้อย
