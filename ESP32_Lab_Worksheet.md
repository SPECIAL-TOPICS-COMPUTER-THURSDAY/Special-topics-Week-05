...
-SRAM Memory ใช้เก็บข้อมูลที่เปลี่ยนแปลงได้ระหว่างการทำงาน / Flash Memory ใช้เก็บข้อมูลถาวรที่ไม่เปลี่ยน
2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
- Stack Variable อยู่ในช่วงที่เป็น Internal SRAM ประมาณ 0x3ffbxxxx , Global และ Static Variable เก็บใน Internal SRAM เช่นกัน  อยู่ในช่วง 0x3ffbxxxx, Heap Allocation ก็อยู่ใน Internal SRAM ช่วง 0x3ffbxxxx, Constant Data เช่น string literals หรือข้อมูลคงที่อื่นๆ จะถูกเก็บใน Flash Memory อยู่ในช่วงประมาณ 0x3f40xxxx 
3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
- Flash Memory: ขนาดทั้งหมด: 2MB , ใช้ไป (app binary): 0x27ea0 ≈ 161,952 bytes (~158 KiB) ,ส่วนที่เหลือใน partition: 0xd8160 ≈ 886,032 bytes (~865 KiB)
---
...
|-----------|-------------|-----------|-------------------|
| Sequential | Internal SRAM | 8727 | 1.00x |
| Random | Internal SRAM | 10378 | 1.19x |
| Sequential | External Memory | 35468 | 4.07x |
| Random | External Memory | 33270 | 3.81x |
...
- Sequential เข้ากันได้ดีมากกับ cache design ทำให้เร็วกว่า random
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
- Internal SRAM คือหน่วยความจำความเร็วสูงในตัว ใช้สำหรับประสิทธิภาพ ในขณะที่ External memory มีขนาดใหญ่กว่าแต่ช้ากว่า
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
- Stride เล็ก → เข้ากันดีกับ cache, เวลาอ่านมาก
- Stride ใหญ่ → อ่านน้อยครั้ง, อาจเร็วขึ้นในบางงานที่ต้องการ sampling ข้อมูล
---
...
| Average Time per Iteration (μs) | 78 | 9729 |
| Total Execution Time (ms) | 4998 | 5947 |
| Task Completion Rate | 20 | 25.2  |
...
| Average Latency (μs) | ~16,000* |
| Queue Overflow Count | 0 |

### คำถามวิเคราะห์

1. **Core Specialization**: จากผลการทดลอง core ไหนเหมาะกับงานประเภทใด?
- จากผลการทดลอง Core 0 เหมาะกับงานที่ต้องการความเร็วสูงและทำงานซ้ำๆ เช่นloop , core 1 เหมาะกับงานที่ต้องรองรับการรอ
2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
- ประมาณ 16,000 ซึ่งถือว่า  มี overhead สูงพอสมควร จึงควรสื่อสารเมื่อจำเป็น
3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
- แต่ละ core ใช้งานได้ดี และไม่มี core ใดว่างงานนาน แต่ Core 1 ต้องรอข้อความ จึงควรปรับลำดับหรือช่วงเวลาการทำงานของแต่ละ core เพื่อให้ทั้งสอง core ทำงานได้สมดุลยิ่งขึ้น
