...
1. **Memory Types**: SRAM และ Flash Memory ใช้เก็บข้อมูลประเภทไหน?
    SRAM (Static RAM) หน้าที่: เก็บข้อมูลชั่วคราว (volatile) ลักษณะ: ข้อมูลจะหายเมื่อรีเซ็ตหรือปิดเครื่อง
    Flash Memory หน้าที่: เก็บข้อมูลถาวร (non-volatile) ลักษณะ: ข้อมูลยังอยู่แม้ปิดเครื่อง
2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
    SRAM (Static RAM) อยู่ใน address range ประมาณ 0x3FFB0000 ถึง 0x3FFEFFFF
    Flash Memory อยู่ใน address range เช่น 0x10000 ขึ้นไป (สำหรับ code), 0x3F400000 (สำหรับข้อมูล), 0x9000 (สำหรับ NVS)
3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
    มีหน่วยความจำทั้งหมดประมาณ 520 KB (Internal SRAM)
    ใช้ไปแล้ว: 380,136 bytes ≈ 371 KB
---
...
|-----------|-------------|-----------|-------------------|
| Sequential | Internal SRAM | 57 | 1.00x |
| Random | Internal SRAM | 	114 | 2.00x |
| Sequential | External Memory | 	9500 | 166.67x |
| Random | External Memory | 9667 | 169.60x |
...
1. **Cache Efficiency**: ทำไม sequential access เร็วกว่า random access?
    การเข้าถึงแบบ sequential (เรียงตามลำดับ) ทำให้ CPU สามารถดึงข้อมูลหลายรายการมาเก็บใน cache ได้พร้อมกัน (cache line prefetching)
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
    Internal SRAM  อยู่ภายในชิป เข้าถึงเร็ว (หน่วย ns) ขนาดเล็ก เหมาะสำหรับข้อมูลที่ใช้งานบ่อย
    External Memory  อยู่นอกชิป (เชื่อมผ่าน bus เช่น SPI/QSPI) เข้าถึงช้า (หน่วย μs) ขนาดใหญ่  เหมาะสำหรับเก็บข้อมูลขนาดใหญ่
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
เมื่อ stride ขนาดเล็ก (เช่น 1, 2) → ข้อมูลอยู่ใกล้กัน → cache prefetch ทำงานได้ดี
เมื่อ stride ขนาดใหญ่ (เช่น 8, 16) → ข้อมูลอยู่ห่างกันมาก → cache miss มากขึ้น → ต้องโหลดจาก RAM → performance ลดลง
---
...
| Average Time per Iteration (μs) | 55 | 9686 |
| Total Execution Time (ms) | 5000 | 5941 |
| Task Completion Rate | 20 | 25 |
...
| Average Latency (μs) | 13732 |
| Queue Overflow Count | 0 |

### คำถามวิเคราะห์

1. **Core Specialization**: จากผลการทดลอง core ไหนเหมาะกับงานประเภทใด?
   งานที่ต้องการความถี่สูงและความเร็วในการประมวลผลรอบสั้น เช่น loop ควบคุม, งานประมวลผลเบาๆ, หรือ polling ที่ต้องทำซ้ำบ่อย
2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
    ค่าเฉลี่ย latency ที่วัดได้ ≈ 13,732 μs (หรือประมาณ 13.7 ms)
3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
    ไม่สมดุลแต่ยังอยู่ในระดับที่พอรับได้
---
...
1. เปรียบเทียบประสบการณ์การใช้ Docker ในสัปดาห์นี้กับสัปดาห์ที่ 4:
   สามารถใช้งานได้ดีขึ้น เป็นมากขึ้น และสามารถเข้าทำคอนเทนเนอร์ได้ดี
2. สิ่งที่เรียนรู้เพิ่มเติมเกี่ยวกับ ESP32 architecture:
   ESP32 มี 2 คอร์ประมวลผล
Core 0 (PRO_CPU): ใช้สำหรับระบบหลัก เช่น Wi-Fi stack, OS task
Core 1 (APP_CPU): ใช้สำหรับแอปพลิเคชันของผู้ใช้
ทำให้สามารถรัน หลาย task พร้อมกัน ได้อย่างมีประสิทธิภาพ
3. ความท้าทายที่พบในการทำ architecture analysis:
   การใช้คำสั่ง build และการสร้างไฟล์แก้ไขไฟล์เป็นตามโครงสร้างที่ถูกต้อง
