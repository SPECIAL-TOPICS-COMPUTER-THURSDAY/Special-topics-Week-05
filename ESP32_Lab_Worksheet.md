...
1. **Docker Commands**: คำสั่ง `docker-compose up -d` และ `docker-compose exec esp32-dev bash` ทำอะไร?
    docker-compose up -d สร้างและรัน container แบบเบื้องหลัง (detached mode)
    docker-compose exec esp32-dev bash เข้าสู่ shell ของ container ชื่อ esp32-dev เพื่อใช้งานภายใน
...
3. **New Tools**: เครื่องมือใหม่ที่ติดตั้ง (tree, htop) ใช้ทำอะไร?
    tree แสดงโครงสร้างไฟล์แบบเป็นลำดับชั้น
    htop ดูการใช้ทรัพยากรระบบแบบ real-time
4. **Architecture Focus**: การศึกษา ESP32 architecture แตกต่างจากการทำ arithmetic ใน Lab4 อย่างไร?
ESP32 architecture มี hardware หลาย core, interrupt, register, และ peripheral จริง
Lab4 arithmetic เป็นแค่การคำนวณเลขบน software ไม่เกี่ยวกับ hardware จริง
...
1. **Memory Types**: SRAM และ Flash Memory ใช้เก็บข้อมูลประเภทไหน?
    SRAM ใช้เก็บ ข้อมูลที่เปลี่ยนแปลงได้ขณะรันโปรแกรม เช่น ตัวแปรแบบ global, local (stack), heap, และ buffer
    Flash Memory ใช้เก็บ ข้อมูลถาวร เช่น โค้ดโปรแกรม, ตัวแปร const, และ string literal ที่ไม่เปลี่ยนค่า
2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
    Flash: 0x3f400000 ขึ้นไป
    SRAM: 0x3ffb0000 ขึ้นไป
3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
    Internal SRAM รวม: 520,192 bytes
    Free Heap (ยังไม่ใช้): 303,088 bytes
    Memory ที่ใช้ไปแล้ว = 520,192 - 303,088 = 217,104 bytes
---
...
|-----------|-------------|-----------|-------------------|
| Sequential | Internal SRAM | 5385 | 1.00x |
| Random | Internal SRAM | 6301 | 1.17x |
| Sequential | External Memory | 5271 | 0.98x |
| Random | External Memory | 5959 | 1.13x |
...
1. **Cache Efficiency**: ทำไม sequential access เร็วกว่า random access?
    Cache ทำงานได้เต็มประสิทธิภาพ — เมื่อเข้าถึงข้อมูลต่อเนื่องกัน (sequential), cache จะโหลดข้อมูลล่วงหน้า(prefetch) มาเก็บไว้ในบล็อก ทำให้การเข้าถึงเร็วขึ้น 
    ลดการ miss ของ cache — การเข้าถึงแบบกระโดด (random) ทำให้เกิด cache miss บ่อย ส่งผลให้ต้องโหลดข้อมูลใหม่จากหน่วยความจำหลักซึ่งช้ากว่า
...
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
    Internal SRAM เป็นหน่วยความจำที่อยู่ภายในชิป (on-chip) ทำงานเร็ว เข้าถึงได้โดยตรง เหมาะสำหรับงานที่ต้องการ ความเร็วสูง เช่น ตัวแปรสำคัญหรือ buffer ขนาดเล็กที่ใช้บ่อย
    External PSRAM เป็นหน่วยความจำภายนอกที่เชื่อมต่อผ่าน SPI bus ทำงานช้ากว่า เข้าถึงผ่าน memory controller เหมาะสำหรับเก็บ ข้อมูลจำนวนมาก ที่ไม่ต้องใช้งานอย่างต่อเนื่อง เช่น ภาพ, log, หรือ buffer ขนาดใหญ่
...
| Average Time per Iteration (μs) | 66 | 9607 |
| Total Execution Time (ms) | 4995 | 5936 |
| Task Completion Rate | 20 iter/s | 25.3 iter/s |
...
| Messages Sent | 9 |
| Messages Received | 9 |
| Average Latency (μs) | 12034 |
| Queue Overflow Count | 0 |

### คำถามวิเคราะห์

1. **Core Specialization**: จากผลการทดลอง core ไหนเหมาะกับงานประเภทใด?
    Core 0 (PRO_CPU) ทำงานเร็วกว่า (66 μs/iteration) เหมาะกับ งานประมวลผลหนัก ความเร็วสูง
    Core 1 (APP_CPU) ใช้เวลามากกว่า (9607 μs/iteration) เหมาะกับ งานรอข้อมูล หรืองาน background
2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
    มี latency เฉลี่ย ~12,034 μs ต่อข้อความ
3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
    Core 1 ทำงาน หนักกว่าและช้ากว่า, มี queue ค้าง ไม่สมดุล 
