...
| Heap | heap_ptr | 0xffb526c | SRAM |

**Table 2.2: Memory Usage Summary**

| Memory Type | Free Size (bytes) | Total Size (bytes) |
|-------------|-------------------|--------------------|
| Internal SRAM | 303088 | 520,192 |
...
- SRAM ใช้เก็บข้อมูลที่เปลี่ยนแปลงได้ขณะโปรแกรมทำงาน เช่น ตัวแปรใน Stack (ตัวแปร local ในฟังก์ชัน) ตัวแปร Global หรือ Static ที่อยู่ใน RAM ข้อมูลที่จัดสรรผ่าน Heap (malloc, new)
- Flash Memory ใช้เก็บข้อมูลถาวร  เช่น โค้ดโปรแกรม และ ค่าคงที่ (const) ตัวแปร const หรือ string literal ("Hello") ที่ประกาศให้เก็บใน flash จะอยู่ตรงนี้
2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
- Flash 0x3f400000 ขึ้นไป 
- SRAM: 0x3ffb0000 ขึ้นไป
3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
- Internal SRAM = 520,192 bytes Free = 303,088 bytes 520,192 − 303,088 = 217,104 bytes
---
...
|-----------|-------------|-----------|-------------------|
| Sequential | Internal SRAM | 5203 | 1.00x |
| Random | Internal SRAM | 5491 | 1.06x |
| Sequential | External Memory | 5327 | 1.00x |
| Random | External Memory | 5449 | 1.02x |
...
- Sequential access อ่านข้อมูลในลำดับต่อเนื่อง CPU และ cache controller สามารถ prefetch ข้อมูลล่วงหน้าเป็นบล็อกใหญ่ cache line ได้ ทำให้ cache hit rate สูง และลดจำนวนครั้งที่ต้องไปดึงข้อมูลจากหน่วยความจำหลัก SRAM/Flash
- Random access กระโดดไปอ่านตำแหน่งกระจาย การ prefetch ไม่ได้ผล เพราะตำแหน่งถัดไปคาดเดาไม่ได้ ต้องโหลดข้อมูลจากหน่วยความจำหลายครั้ง cache miss มากขึ้นใช้เวลาเฉลี่ยต่อการเข้าถึงสูงขึ้น
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
- Internal SRAM = หน่วยความจำหลักที่อยู่ในตัวชิป ESP32 เลย เข้าถึงได้เร็วมาก CPU ต่อถึงโดยตรง ใช้เก็บตัวแปรสำคัญ, ตัวแปร local, stack, heap ขนาดเล็ก กินไฟน้อยและหน่วงเวลาต่ำมาก
- External Memory (PSRAM, SPI RAM) = หน่วยความจำต่อเพิ่มภายนอกชิป ผ่านบัส SPI/QSPI ความเร็วต่ำกว่า เพราะต้องส่งสัญญาณออกนอกชิป
มี latency สูงกว่า เหมาะสำหรับเก็บข้อมูลขนาดใหญ่ เช่น buffer ของภาพ เสียง หรือข้อมูลที่ไม่ต้องเข้าถึงบ่อย
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
- Stride ใหญ่ทำให้เวลาลดลงเพราะ ลดจำนวน access และทำให้ cache ใช้ประโยชน์ได้เต็มที่ แต่ในงานจริงถ้าต้องการอ่านข้อมูลทุก element จะไม่ได้เร็วขึ้นเสมอ
---
...
| Average Time per Iteration (μs) | 44 | 9,611 |
| Total Execution Time (ms) | 4,994 | 5,936 |
| Task Completion Rate | 20.0 | 25.3 |
...
| Average Latency (μs) | 12,200.5 |
| Queue Overflow Count | 0 |

### คำถามวิเคราะห์

1. **Core Specialization**: จากผลการทดลอง core ไหนเหมาะกับงานประเภทใด?
- Core 0 (PRO_CPU) เหมาะกับงานเบา, เร็ว, ถี่ high-frequency, low-latency เช่น interrupt handling, message producer, polling, หรืองานที่ต้องตอบสนองทันที
- Core 1 (APP_CPU) เหมาะกับงานหนัก/บล็อก/ประมวลผลเชิงคำนวณหรือ I/O ที่ช้ากว่า เช่น การประมวลผลข้อมูลภาพ เสียง การเชื่อมต่อเครือข่าย หรืองานที่ต้องทำงานแบบ synchronous
2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
- ค่า overhead การสื่อสารข้ามคอร์ในเทสนี้ประมาณ 12 ms 
3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
-  จาก log คิว Queue messages waiting แทบไม่เกิน 1 และ ไม่มี queue overflow — แปลว่า สำหรับอัตรางานปัจจุบัน ระบบยังรับได้ แต่มี margin ต่ำ หากโหลดเพิ่มขึ้นจะเกิด backlog และ latency เพิ่มขึ้น
...
**ข้อมูลนักศึกษา:**
- ชื่อ: นางสาวสุวรรณารัตน์ ปัดคำ
- รหัสนักศึกษา: 66030196
- วันที่ทำการทดลอง: 9/8/2568
**Checklist การทดลอง:**
...
1. เปรียบเทียบประสบการณ์การใช้ Docker ในสัปดาห์นี้กับสัปดาห์ที่ 4:
   สัปดาห์ที่ 4 ยังต้องใช้เวลามากกับการตั้งค่า environment และแก้ปัญหา dependency ต่าง ๆ แต่ในสัปดาห์นี้การใช้งานดีขึ้นเพราะมี image และ container พร้อมใช้แล้ว

2. สิ่งที่เรียนรู้เพิ่มเติมเกี่ยวกับ ESP32 architecture
  เข้าใจโครงสร้างหน่วยความจำมากขึ้น ทั้ง Internal SRAM ที่เชื่อมตรงกับ CPU latency ต่ำ ความเร็วสูง และ External Memory PSRAM/SPI RAM ที่มี latency สูงกว่าเพราะผ่านบัสภายนอก รวมถึงการทำงานแบบ multi-core ที่มี Core0 PRO_CPU กับ Core1 APP_CPU และวิธีสื่อสารกันผ่าน queue, task notification, หรือ shared buffer

3. ความท้าทายที่พบในการทำ architecture analysis:
   การเก็บข้อมูลเชิงปริมาณที่แม่นยำ เช่น latency และ throughput ของแต่ละ core ต้องระวัง noise จากการรันจริง การวิเคราะห์ load balancing ต้องใช้ข้อมูลหลายมิติ queue length, utilization, context switch และต้องตีความผลให้สัมพันธ์กับข้อจำกัดของระบบจริง
