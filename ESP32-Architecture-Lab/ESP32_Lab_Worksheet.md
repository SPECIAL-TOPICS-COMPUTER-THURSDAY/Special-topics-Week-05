...
ANS - docker-compose up -d	สร้างและรัน container ตาม docker-compose.yml ในโหมดพื้นหลัง

- docker-compose exec esp32-dev bash	เข้าไปใช้ shell (bash) ภายในคอนเทนเนอร์ชื่อ esp32-dev
...
ANS สร้างโครงสร้าง irectory

4. **Architecture Focus**: การศึกษา ESP32 architecture แตกต่างจากการทำ arithmetic ใน Lab4 อย่างไร?
ANS ESP32 architecture    =เข้าใจการทำงานภายในของฮาร์ดแวร์

Lab4: Arithmetic = ฝึกใช้โค้ดควบคุมอุปกรณ์ด้วยคณิตศาสตร์
...
ANS SRAM (Static RAM)
ใช้เก็บ ข้อมูลขณะโปรแกรมกำลังทำงาน

Flash Memory (SPI Flash)
ใช้เก็บ ข้อมูลถาวร ที่ไม่หายเมื่อปิดเครื่อง

2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
ANS ประเภทตัวแปร int x; (local)	Stack (ใน DRAM)	0x3FFBxxxx
...
|=== Test 2: External Memory Access ===
External Sequential Access: 31903 μs (sum=3609780224)
External Random Access: 43068 μs (sum=3609780224)
External Memory Performance Ratio: 1.35x

Memory Type Comparison (Sequential Access):
External/Internal Speed Ratio: 4.95x

=== Test 3: Stride Access Patterns ===
Stride 1 (stride 1): 9237 μs (sum=3609780224)
Stride 2 (stride 2): 3064 μs (sum=1801306112)
Stride 4 (stride 4): 1790 μs (sum=3044552704)
Stride 8 (stride 8): 904 μs (sum=3666176000)
Stride 16 (stride 16): 607 μs (sum=1829504000)

Stride Analysis:
Stride 2/1 ratio: 0.33x
Stride 4/1 ratio: 0.19x
Stride 8/1 ratio: 0.10x
Stride 16/1 ratio: 0.07x

Cache performance analysis complete!
I (7607) main_task: Returned from app_main()


### คำถามวิเคราะห์

1. **Cache Efficiency**: ทำไม sequential access เร็วกว่า random access?
Sequential access ดึงข้อมูลแบบต่อเนื่องใน cache line ทำให้เร็ว
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
Internal SRAM เร็วแต่เล็ก / External memory ใหญ่แต่ช้า
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
Stride เล็ก → cache friendly / Stride ใหญ่ → cache miss เยอะ
...
| Average Time per Iteration (μs) | 54 μs | 9659 μs |
| Total Execution Time (ms) | 	4997 ms | 	5939 ms |
...
| Average Latency (μs) | 14,000 μs |
| Queue Overflow Count | 	0|

### คำถามวิเคราะห์

1. **Core Specialization**: จากผลการทดลอง core ไหนเหมาะกับงานประเภทใด?
ANS Core 0 = real-time, Core 1 = processing

2. **Communication Overhead**: inter-core 
communication มี overhead เท่าไร?
ANS สูง (~14 ms/ข้อความ) แต่ไม่ล้น

3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
ANS ทำได้ดีระดับหนึ่ง แต่ Core 1 รับงานหนักกว่า
---
